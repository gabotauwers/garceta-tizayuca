...
class AreaDB{
    ...
    public function getProductos(){
        ...
        $dbh = $conexion->getDbh();
        try{
            $consulta = "Select CodigoProd,NombreProd,DescripcionProd,Cantidad,Precio,ImagenProd,tipomaterial.NombreTMa,tipomueble.NombreTMu,area.NombreArea FROM producto INNER JOIN tipomaterial,tipomueble,area ON producto.IdTipoMaterial = tipomaterial.IdTipoMaterial, producto.idTipoMueble = tipomueble.idTipoMueble, producto.IdArea = area.IdArea";
    ...
    public function getProductoID($id){
        ...
        $dbh = $conexion->getDbh();
        try{
            $consulta = "Select CodigoProd,NombreProd,DescripcionProd,Cantidad,Precio,ImagenProd,tipomaterial.NombreTMa,tipomueble.NombreTMu,area.NombreArea FROM producto INNER JOIN tipomaterial,tipomueble,area ON producto.IdTipoMaterial = tipomaterial.IdTipoMaterial, producto.idTipoMueble = tipomueble.idTipoMueble, producto.IdArea = area.IdArea WHERE IdProducto = ?";
            $stmt = $dbh->prepare($consulta);
            $stmt->setFetchMode(PDO::FETCH_BOTH);
            $stmt->bindParam(1, $id);
            $stmt->execute();
